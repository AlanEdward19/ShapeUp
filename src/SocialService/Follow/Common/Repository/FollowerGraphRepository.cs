...
public class FollowerGraphRepository(GraphContext graphContext) : IFollowerGraphRepository
{
   ...
   /// <param name="followedProfileId"></param>
    ...
    {
        var query = $"MATCH (a:Profile {{id: '{followerProfileId}'}}), (b:Profile {{id: '{followedProfileId}'}}) CREATE (a)-[:FOLLOWING]->(b)";
    ...
    {
        var query = $"MATCH (a:Profile {{id: '{followerProfileId}'}})-[r:FOLLOWING]->(b:Profile {{id: '{followedProfileId}'}}) DELETE r";
