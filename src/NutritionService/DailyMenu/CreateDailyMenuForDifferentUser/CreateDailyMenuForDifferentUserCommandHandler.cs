...
/// <summary>
/// Handles the creation of a daily menu for a different user.
/// </summary>
public class CreateDailyMenuForDifferentUserCommandHandler : IHandler<DailyMenuDto, CreateDailyMenuForDifferentUserCommand>
{
    ...
    public CreateDailyMenuForDifferentUserCommandHandler(
    ...
    /// </summary>
    public async Task<DailyMenuDto> HandleAsync(CreateDailyMenuForDifferentUserCommand item, CancellationToken cancellationToken)
    {
        // 1. Criar a entidade DailyMenu com as referências (MealIds)
        var dailyMenu = new DailyMenu(item.DayOfWeek, item.MealIds);
        dailyMenu.SetId();
        dailyMenu.SetCreatedBy(ProfileContext.ProfileId); // Criado pelo profissional logado
        dailyMenu.SetUserId(item.UserId);                 // Atribuído ao usuário alvo
