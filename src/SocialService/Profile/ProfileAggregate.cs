...
public class ProfileAggregate(Profile profile)
{
    ...
    {
        if (!string.IsNullOrWhiteSpace(imageUrl) && (ImageUrl == null || !ImageUrl.Equals(imageUrl, StringComparison.InvariantCultureIgnoreCase)))
            ImageUrl = imageUrl;
    }
    ...
    {
        if (!string.IsNullOrWhiteSpace(bio) && (Bio == null || !Bio.Equals(bio, StringComparison.InvariantCultureIgnoreCase)))
            Bio = bio;
    }
