...
public class CreateMealForDifferentUserCommandHandler : IHandler<MealDto, CreateMealForDifferentUserCommand>
{
    ...
    public CreateMealForDifferentUserCommandHandler(
    ...
    /// <summary>
    /// Handles the creation of a new meal for a different user.
    /// </summary>
    public async Task<MealDto> HandleAsync(CreateMealForDifferentUserCommand item, CancellationToken cancellationToken)
    {
        ...
        meal.SetId();
        meal.SetCreatedBy(ProfileContext.ProfileId); // Criado pelo profissional/usuário logado
        meal.SetUserId(item.UserId);                 // Atribuído ao usuário alvo
