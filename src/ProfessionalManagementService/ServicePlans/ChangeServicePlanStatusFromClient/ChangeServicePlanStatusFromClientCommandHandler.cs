...
public class ChangeServicePlanStatusFromClientCommandHandler(DatabaseContext dbContext) : IHandler<ClientDto, ChangeServicePlanStatusFromClientCommand>
{
    public async Task<ClientDto> HandleAsync(ChangeServicePlanStatusFromClientCommand command, CancellationToken cancellationToken)
    {
        ...
        if(!client.Id.Equals(command.LoggedInUserId) && !servicePlan.ProfessionalId.Equals(command.LoggedInUserId))
            throw new UnauthorizedAccessException("You do not have permission to update this service plan from the client.");
        ...
        catch (Exception e)
        {
            ...
            throw new InvalidOperationException($"An error occurred while updating ServicePlan to Client: {e.Message}", e);
        }
    }
}